...
class ZoomLayout @JvmOverloads constructor(
    ...
    private val mChildRect = RectF()
    private var mHasClickableChildren = false
    ...
    override fun addView(child: View, index: Int, params: ViewGroup.LayoutParams) {
        if (childCount == 0) {
            child.viewTreeObserver.addOnGlobalLayoutListener {
                mChildRect[0f, 0f, child.width.toFloat()] = child.height.toFloat()
                engine.setContentSize(mChildRect)
            }
    ...
    override fun onInterceptTouchEvent(ev: MotionEvent): Boolean {
        return engine.onInterceptTouchEvent(ev) || mHasClickableChildren && super.onInterceptTouchEvent(
            ev
        )
    }
    ...
    override fun computeHorizontalScrollOffset(): Int {
        return (-1 * engine.getPanX() * engine.getRealZoom()).toInt()
    }

    override fun computeHorizontalScrollRange(): Int {
        return (mChildRect.width() * engine.getRealZoom()).toInt()
    }

    override fun computeVerticalScrollOffset(): Int {
        return (-1 * engine.getPanY() * engine.getRealZoom()).toInt()
    }

    override fun computeVerticalScrollRange(): Int {
        return (mChildRect.height() * engine.getRealZoom()).toInt()
    }
    ...
    init {
        ...
        a.recycle()
        engine = ZoomEngine(context, this, this,ZoomApi)
